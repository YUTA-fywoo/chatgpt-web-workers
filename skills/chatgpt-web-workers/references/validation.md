...
Current routing policy overrides historical capability results: read replies through internal tools; send all messages and operate Chat controls through the webpage. A past successful internal send is not authorization to use that route or retest it. This policy update is not a new live capability test.
...
| Direct text follow-up (historical route) | Passed. The correct new reply continued the previous result, and the browser still showed 极高. | Superseded: send follow-ups through the webpage. |
...
| Local file link in direct prompt (historical route) | Failed to transfer a file. No attachment appeared; the worker correctly reported the local path unavailable. | Paste suitable short text into the webpage composer, or upload an actual file. |
...
| Direct follow-up using prior attachments (historical route) | Passed. Without another upload, the second worker recovered the same nonce and values and returned the correct squared total, 17161. | Reuse the attachments in that Chat; send follow-ups through its webpage. |
...
Recorded tool boundaries: `create_thread` did not expose an ordinary Chat creation target; internal sending exposed no attachment field; model/thinking overrides applied to Codex targets; `wait_threads` documented Codex workers. Use current tool schemas and the entrypoint's routing policy rather than treating these historical observations as a send fallback.
